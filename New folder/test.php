...
require_once 'PHPExcel/IOFactory.php';
	...
	for ($i = 1; $i <=3; $i++) {
		$s=file_get_contents('https://www.lazada.vn/catalog/?_keyori=ss&from=input&page='.$i.'&q=day%20guitar');
	...
	function getItem($url)
	{
		...
		// print_r($name[1]);
		$name=str_replace(' ','-',convert($name[1]));
		$name_img=str_split($name,10);
		// print_r($name_img[0]);	
		// print_r($name);w
			//Gía
		...
		imagejpeg($image, "c://xampp/htdocs/wordpress-4.9.2/wordpress/wp-content/uploads/2018/05/".$name_img[0].".jpg");
		// print_r($image[1]);
		writeExcel($name,$prices[2][0],$prices[2][1],$name_img[0]);
	}
	...
	function writeExcel($name,$prices,$prices_sale,$name_img)
	{
		...
		->setCellValue('AC'.$i,'http://localhost/wordpress-4.9.2/wordpress/wp-content/uploads/2018/05/'.$name_img.".jpg");
